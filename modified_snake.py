...
class AStar:
    ...
    class Spot:
        ...
        def addNeighbors(self, grid):
            ...
            if i < self.aStar.cols - 2:
                self.neighbors.append(self.aStar.grid[i + 1][j])
            if i > 1:
                self.neighbors.append(self.aStar.grid[i - 1][j])
            if j < self.aStar.rows - 2:
    ...
    def main(self, startX, startY, endX, endY, snakeX, snakeY):
        ...
        # Dividing by 20 bcz it is blocksize, and make it like index for A*
        ...
        while not self.gameExit:
            ...
            if self.status:
                ...
                if len(self.openSet) > 0:
                    # winner is the index in openset with smallest f value
                    winner = 0
...
class Snake:
    ...
    def __init__(self, x, y, FPS):
        ...
        # self.keyX = True      #new Comment
        # self.keyY = True      #new Comment
    ...
    def snake_run(self):
        ...
        gameExit = False
        # food_flag = True      #new Comment

        # foodX = randrange(borderSize, displayX - (2 * borderSize) + blockSize, blockSize)     #new Comment
        # foodY = randrange(borderSize, displayY - (2 * borderSize) + blockSize, blockSize)     #new Comment

        #  If Food generate in snake's body
        ...
        # pathXY = tempStar.main(self.snakeX[0], self.snakeY[0], foodX, foodY, self.snakeX, self.snakeY)
        pathX = pathXY[0]
        pathY = pathXY[1]
        tempCount = 0
        ...
        while not gameExit:
            ...
            # elif self.lead_x == pathX[-1] and self.lead_y == pathY[-1]:
            #     # print("happens")
            #     tempStar = AStar()
            #     pathXY = tempStar.main(self.lead_x, self.lead_y, foodX, foodY, self.snakeX, self.snakeY)
            #     # print("Color")
            ...
            # if len(set(mergeXY[1:])) <= len(mergeXY[1:]) <= len(set(mergeXY[1:])) + 10:       #new Comment
            #     pass

            if len(mergeXY) <= len(set(mergeXY)) + 10:
                pass
